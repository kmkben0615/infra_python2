#Step05_function.py

"""function type
       -특정시점에 여러줄의 명령어를 code를 일괄실행하고자 할때 사용한다.
       -함수도 data이다 (변수에 담을 수 있다)
       -함수안에 저장된 code를 일괄실행하는 것을 함수로 call 한다고 이야기한다.
       -함수는 저장된 code를 다 실행하고 나면 원래 call했던 위치로 실행의 흐름이 넘어온다
       -call 했던 위치로 돌아오면서 어떤 data를 반드시 가죠온다"""

def test1():
    print("test1() 호출됨")

test1()
result1=test1()
#매개변수가 선언되어 있는 함수
#매개변수가 대입할 값을 전달해야 호출할 수가 있다
#매개변수의 이름은 마음대로 지을 수 있다
def test2(arg):
    print("전달빋은 내용:",arg)
    print(f"전달받은 내용:{arg}")

test2(None)
test2(10)
test2("KIM")

def print_sum(num1:int, num2:int):
    sum = num1+num2
    print(f"{num1} + {num2} = {sum}")

print_sum(10,20)
print_sum(20,40)
#
def print_info(name : str, addr: str):
    print(f"이름은 {name} 이고 주소는 {addr}입니다")
print_info("김민규", "송파구")
#keyword를 이용해서 인자를 전달 할 수도 있다
print_info(addr="송파구", name="김민규")

def get_sum(num1:int, num2:int):
    sum = num1+num2
    return sum

result2 = get_sum(10,20)
print("종료")